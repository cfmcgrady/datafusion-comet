...
| `spark.comet.shuffle.celeborn.async.enabled` | Whether to enable asynchronous push for Celeborn shuffle. When enabled, data is pushed to Celeborn workers asynchronously using background worker threads, which can significantly improve performance for large shuffles. This is automatically enabled when writer.mode is 'async_sort'. | true |
| `spark.comet.shuffle.celeborn.async.maxInFlightPerWorker` | Maximum number of in-flight push requests per Celeborn worker. This limits concurrent requests to each worker to prevent overwhelming it. Default is 32. | 32 |
| `spark.comet.shuffle.celeborn.async.numWorkers` | Number of background worker threads for asynchronous push to Celeborn. More workers can improve throughput but use more resources. Default is 4. | 4 |
| `spark.comet.shuffle.celeborn.async.queueCapacity` | Capacity of the task queue for asynchronous push. When the queue is full, the producer will block until space is available, providing natural backpressure. Default is 1000. | 1000 |
| `spark.comet.shuffle.celeborn.compression.codec` | Compression codec for Celeborn shuffle data. Supported values: 'lz4', 'zstd', 'none'. LZ4 offers faster compression/decompression, while ZSTD provides better compression ratio. | lz4 |
| `spark.comet.shuffle.celeborn.enabled` | Whether to enable Celeborn shuffle for Comet. When enabled, Comet will use Apache Celeborn for distributed shuffle operations instead of writing to local disk. This requires a running Celeborn cluster and proper configuration of master endpoints and LifecycleManager address. | false |
| `spark.comet.shuffle.celeborn.fetch.maxReqsInFlight` | Maximum number of concurrent fetch requests from Celeborn workers. | 3 |
| `spark.comet.shuffle.celeborn.lifecycleManager.host` | The host address of the LifecycleManager running on the Spark Driver. This is typically set automatically by the Celeborn Spark plugin. Required when Celeborn shuffle is enabled. |  |
| `spark.comet.shuffle.celeborn.lifecycleManager.port` | The port of the LifecycleManager running on the Spark Driver. Default is 9098. | 9098 |
| `spark.comet.shuffle.celeborn.master.endpoints` | Comma-separated list of Celeborn master endpoints (e.g., 'host1:9097,host2:9097'). Required when Celeborn shuffle is enabled. |  |
| `spark.comet.shuffle.celeborn.push.bufferSize` | The buffer size for pushing data to Celeborn workers. Larger buffer sizes can improve throughput but use more memory. | 4194304b |
| `spark.comet.shuffle.celeborn.push.maxReqsInFlight` | Maximum number of concurrent push requests to Celeborn workers. | 32 |
| `spark.comet.shuffle.celeborn.rpc.timeout` | RPC timeout in milliseconds for Celeborn operations. | 30000 |
| `spark.comet.shuffle.celeborn.sort.memoryThreshold` | Memory threshold in bytes for sort-based shuffle writer. When accumulated data exceeds this threshold, records are sorted by partition ID and pushed to Celeborn. Larger values reduce push frequency but use more memory. Default is 64MB. | 67108864b |
| `spark.comet.shuffle.celeborn.sort.pushBufferSize` | Push buffer size in bytes for sort-based shuffle writer. When accumulated data for a partition exceeds this size, it is pushed to Celeborn. Larger values reduce push frequency but use more memory. Default is 4MB. | 4194304b |
| `spark.comet.shuffle.celeborn.writer.mode` | Shuffle writer mode for Celeborn integration. 'async_sort' uses async push with background workers for best performance. 'sort' accumulates records, sorts by partition ID, and batches push. 'hash' pushes data immediately per partition (original behavior). Default is 'async_sort'. | async_sort |
